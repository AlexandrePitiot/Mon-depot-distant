...
$pdoStat = $pdo ->prepare("INSERT INTO produits(pro_ref, pro_cat_id, libelleProduit, pro_description, pro_prix, pro_stock, pro_couleur, pro_bloque, pro_d_ajout)
VALUES(:pro_ref, :pro_cat_id, :libelleProduit, :pro_description, :pro_prix, :pro_stock, :pro_couleur, :pro_bloque, '".$date."')");
...
$pdoStat->bindValue(':libelleProduit', $_POST['libelle'], PDO::PARAM_STR);
...
$cheminEtNomDefinitif = '../assets/images/jarditou_photos'.$nouveauNom;
...
if($InsertIsOk2){
    //$message = "Le produit a été rajouté dans la base de données";
    header("Location: ../View/tableau.php"); // Si le produit a bien été ajouté, il y a une redirection ver le tableau.php
}
else{
...
if (!empty($aErreur)) // Si le tableau n'est pas vide
{
    ...
    header("Location:../Controller/produits_ajout.php?".$sUrl); // On affiche les erreurs dans le formulaire formulaire.php
