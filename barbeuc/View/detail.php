...
include("../View/entete.php"); // Inclusion de l'en-tête construite dans le fichier entete.php
require "../Model/connexion_bdd.php"; // Inclusion de notre bibliothèque de fonctions
...
<form class="col-lg-12" action="../Controller/produits_ajout_script.php" method="post"> <!-- Action vers le fichier php ou on écrit le script à exécuter en méthode POST -->

<div class="text-center">

<img src="../assets/images/jarditou_photos/<?=$produit->pro_id?>" width="300" alt="produit"> <!-- Pour ajouter la photo du produit : width="300 permet de redimensionner la photo et en n'indiquant qu'un seul paramètre le navigateur se charge de calculer le deuxième c'est à dire height en conservant les proportions de départ -->
</div>
...
<div class="form-group">
    ...
    <input type="text" class="form-control" name="libelle" value ="<?=$produit->libelleProduit?>" id="libelle" readonly>  
</div>
...
<div class="form-group">
    ...
    <a href="../View/tableau.php" class="btn btn-dark m-0">Retour</a>
<!-- <a href="/php/produits_modif_script.php?id=<?php echo $r['pro_id']?>" class="btn btn-warning">EDITER</a> -->
    ...
    //     {
        ...
        <!-- Quand on clique sur le bouton modifier on exécute le script du fichier sur lequel on fait un lien et on récupère l'ID avec ?pro_id=<?= $produit->pro_id?> -->
         <a href="../Controller/produits_modif.php?pro_id=<?= $produit->pro_id?>" class="btn btn-warning m-0">Modifier</a>
        ...
        <a href="../Controller/produits_supprimes_script.php?pro_id=<?= $produit->pro_id?>" class="btn btn-danger m-0" onclick="return confirm('Etes-vous certain(e) de vouloir supprimer le produit ?')">Supprimer</a>
        <!-- Quand on clique sur le bouton ajouter on exécute le script du fichier sur lequel on fait un lien et on récupère l'ID avec ?pro_id=<?=$produit->pro_id?> -->
        <a href="../View/panier.php?action=ajout&l=LIBELLEPRODUIT&q=QUANTITEPRODUIT&p=PRIXPRODUIT=<?= $produit->pro_id?>" class="btn btn-warning m-0">Ajouter au panier</a>        <?php
        // }
    ?>
</div>
...
<?php
include("../View/pieddepage.php"); // Inclusion du pied de page
?>